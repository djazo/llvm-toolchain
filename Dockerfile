...
RUN mkdir -p /data/sysroots ; \
  apk --arch x86_64 -X http://dl-cdn.alpinelinux.org/alpine/v3.12/main -U --allow-untrusted --root /data/sysroots/x86_64 --initdb add alpine-base musl-dev libc-dev linux-headers g++ ; \
  apk --arch armv7 -X http://dl-cdn.alpinelinux.org/alpine/v3.12/main -U --allow-untrusted --root /data/sysroots/armv7 --initdb add alpine-base musl-dev libc-dev linux-headers g++ ; \
  apk --arch aarch64 -X http://dl-cdn.alpinelinux.org/alpine/v3.12/main -U --allow-untrusted --root /data/sysroots/aarch64 --initdb add alpine-base musl-dev libc-dev linux-headers g++
...
COPY --from=wendy /opt/toolchain /opt/toolchain
...
LABEL com.embeddedreality.image.maintainer="arto.kitula@gmail.com" \
  ...
  com.embeddedreality.image.version="11" \
